...
/**
 * @license Copyright 2011-2014 Agate Inc., MIT License
 * see https://github.com/agate/gravityforms-plugin/blob/master/LICENSE
 */

/**
 * Class for managing form data
 */
class GFAgateFormData
{
    public $price    = 0;
    public $total    = 0;
    public $shipping = 0;
    ...
    public function __construct(&$form)
    {
        if (false === isset($form) || true === empty($form)) {
            error_log('[ERROR] In GFAgateFormData::__construct(): Missing or invalid $form parameter.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Missing or invalid $form parameter in the GFAgateFormData::__construct() function.');
        }
    ...
    {
        if (false === isset($form) || true === empty($form)) {
            error_log('[ERROR] In GFAgateFormData::loadForm(): Missing or invalid $form parameter.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Missing or invalid $form parameter in the GFAgateFormData::loadForm() function.');
        }
    ...
    {
        if (false === isset($form) || true === empty($form)) {
            error_log('[ERROR] In GFAgateFormData::getProductPrice(): Missing or invalid $form parameter.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Missing or invalid $form parameter in the GFAgateFormData::getProductPrice() function.');
        }

        if (false === isset($field) || true === empty($field)) {
            error_log('[ERROR] In GFAgateFormData::getProductPrice(): Missing or invalid $field parameter.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Missing or invalid $field parameter in the GFAgateFormData::getProductPrice() function.');
        }
    ...
    {
        if (false === isset($form) || true === empty($form)) {
            error_log('[ERROR] In GFAgateFormData::getShipping(): Missing or invalid $form parameter.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Missing or invalid $form parameter in the GFAgateFormData::getShipping() function.');
        }

        if (false === isset($field) || true === empty($field)) {
            error_log('[ERROR] In GFAgateFormData::getShipping(): Missing or invalid $field parameter.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Missing or invalid $field parameter in the GFAgateFormData::getShipping() function.');
        }
