 ...
 */
class GFAgateException extends Exception {}
class GFAgateCurlException extends Exception {}
...
class GFAgatePlugin
{
    ...
    protected $txResult = null;       // Agate transaction results

    /**
     * Static method for getting the instance of this singleton object
     *
     * @return GFAgatePlugin
     */
    ...
    public function gformValidation($data)
    {
        ...
        if ($data['is_valid']) {
            $formData = new GFAgateFormData($data['form']);

            if (false === isset($formData) || true === empty($formData)) {
                error_log('[ERROR] In GFAgatePlugin::gformValidation(): Could not create a new McryptExtension object.');
                throw new \Exception('An error occurred in the Agate Payment plugin: Could not create a new gformValidation object.');
            }
            ...
            if ($this->hasFormBeenProcessed($data['form'])) {
                ...
                $formData->buyerName['validation_message'] = $this->getErrMsg(GFAGATE_ERROR_ALREADY_SUBMITTED);
                ...
                if (!$formData) {
                    ...
                    $formData->buyerName['failed_validation']  = true;
                    $formData->buyerName['validation_message'] = $this->getErrMsg(GFAGATE_ERROR_NO_AMOUNT);
                } else {
                    if ($formData->total > 0) {
    ...
    protected function hasFormBeenProcessed($form)
    {
        ...
        $sql       = "select lead_id from {$wpdb->prefix}rg_lead_meta where meta_key='gfagate_unique_id' and meta_value = %s";
     ...
     * @param array $data an array with elements is_valid (boolean) and form (array of form elements)
     * @param GFAgateFormData $formData pre-parsed data from $data
     * @return array
     */
    protected function processSinglePayment($data, $formData)
    {
        try {
            $agate = new GFAgatePayment();

            if (false === isset($agate) || true === empty($agate)) {
                error_log('[ERROR] In GFAgatePlugin::processSinglePayment(): Could not create a new GFAgatePayment object.');
                throw new \Exception('An error occurred in the Agate Payment plugin: Could not create a new GFAgatePayment object.');
            }

            $agate->uid                = uniqid();
            $this->uid                = $agate->uid;
            $agate->total              = $formData->total;
            $agate->buyerEmail        = $formData->buyerEmail;

            $this->txResult = array (
                'payment_gateway'    => 'gfagate',
                'gfagate_unique_id' => GFFormsModel::get_form_unique_id($data['form']['id']),
            );

            $response = $agate->processPayment();

            $this->txResult['payment_status']   = 'New';
            $this->txResult['date_created']     = date('Y-m-d H:i:s');
            $this->txResult['payment_date']     = null;
            $this->txResult['payment_amount']   = $agate->total;
            $this->txResult['transaction_id']   = $agate->uid;
            $this->txResult['transaction_type'] = 1;
            $this->txResult['currency']         = GFCommon::get_currency();
            $this->txResult['status']           = 'Active';
            $this->txResult['payment_method']   = 'Agate';
            $this->txResult['is_fulfilled']     = '0';

        } catch (GFAgateException $e) {
            $data['is_valid'] = false;
            $this->txResult   = array('payment_status' => 'Failed',);

            error_log('[ERROR] In GFAgatePlugin::processSinglePayment(): ' . $e->getMessage());
    ...
    public function gformAfterSubmission($entry, $form)
    {
        ...
        $formData = new GFAgateFormData($form);

        if (false === isset($formData) || true === empty($formData)) {
            error_log('[ERROR] In GFAgatePlugin::gformAfterSubmission(): Could not create a new GFAgateFormData object.');
            throw new \Exception('An error occurred in the Agate Payment plugin: Could not create a new GFAgateFormData object.');
        }
        ...
        if (false === empty($this->txResult)) {
            ...
            if (class_exists('RGFormsModel') == true) {
                ...
                throw new Exception('[ERROR] In GFAgatePlugin::gformAfterSubmission(): GFAPI or RGFormsModel won\'t update lead.');
            }

            // record entry's unique ID in database
            $unique_id = RGFormsModel::get_form_unique_id($form['id']);

            gform_update_meta($entry['id'], 'gfagate_transaction_id', $unique_id);

            // record payment gateway
            gform_update_meta($entry['id'], 'payment_gateway', 'gfagate');
        }
    }
    ...
    public function getErrMsg($errName, $useDefault = false)
    {
        static $messages = array (
            GFAGATE_ERROR_ALREADY_SUBMITTED => 'Payment has already been submitted and processed.',
            GFAGATE_ERROR_NO_AMOUNT         => 'This form is missing products or totals',
            GFAGATE_ERROR_FAIL              => 'Error processing Agate transaction',
        );
